...
export default function Step1Protocol() {
  ...
  return (
    <div className="p-6">
      <h3 className="text-base font-medium text-[var(--text-main)] mb-1">{t('wizard.protocol.title')}</h3>
      <p className="text-sm text-[var(--text-muted)] mb-6">
        {t('wizard.protocol.desc')}
      </p>
          ...
          {protocols.map((p) => (
            ...
            >
              ...
              <div className="text-xs text-[var(--text-muted)] mt-1 flex gap-2">
                {p.supportsDiscovery && <span>{t('wizard.protocol.capScan')}</span>}
                {p.supportsLivePolling && <span>{t('wizard.protocol.capPoll')}</span>}
              </div>
            </button>
          ))}
        </div>
